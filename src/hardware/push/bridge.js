// Synthteam hardware bridge for the Ableton Push 3 — all JS, run
// in-process by the game server (see runtime.ts).
//
...
//
// runPushBridge drives an opened AbletonPush until the hardware link
// fails or the signal aborts, then resolves with the reason; the
// runtime owns retries and closing the device.
...
export async function runPushBridge(push, serverUrl, signal) {
  let stopped = false;
  let finishBridge = null;
  ...
  function renderScreen() {
    ...
    if (defend === null && now < missionFlashUntil) {
      ...
      centered(text, (HEIGHT - 7 * 12) / 2 + Math.cos(now / 16) * 5, 12,
        [255, 255, 255]);
      push.display.rgba(buffer).catch((error) => {
        finish(`display link lost — ${errorText(error)}`);
      });
      return;
    }
